<?php class CustomLogoWidget extends WP_Widget
{
  ...
  /* Displays the Widget in the front-end */
	function widget($args, $instance){
		...
		echo $before_widget;
?>
	<p id="footer-logo"><?php echo $logo; ?></p>
...
<?php
	...
	function form($instance){
		...
		# Logo Image
		echo '<p><label for="' . $this->get_field_id('logoImagePath') . '">' . 'Logo Image URL ( leave blank to use site title ):' . '</label><textarea cols="20" rows="2" class="widefat" id="' . $this->get_field_id('logoImagePath') . '" name="' . $this->get_field_name('logoImagePath') . '" >'. esc_textarea( $logoImagePath ) .'</textarea></p>';
		# Text
